...
//        _slideBar = sliderBar;
        sliderBar.slideBarItemSelectedCallback { (idx:UInt) -> Void in
            self.tableView.scrollToRowAtIndexPath(NSIndexPath(forRow: Int(idx), inSection: 0), atScrollPosition: UITableViewScrollPosition.Top, animated: false)
        }
        self.view.addSubview(sliderBar)
...
//    [self.slideBar selectSlideBarItemAtIndex:indexPath.row];
      ...
      self.slideBar.selectSlideBarItemAtIndex(row)
    }
